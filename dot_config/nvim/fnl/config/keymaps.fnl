  ...
  (let [nmap-leader (fn [suffix rhs desc]
    ...
    (leader-maps nmap-leader
      [["ed" "<Cmd>lua MiniFiles.open()<CR>" "Directory"]
       ...
       ["f:" "<Cmd>Pick history scope=\":\"<CR>" "\":\" history"]
       ["fa" "<Cmd>Pick git_hunks scope=\"staged\"<CR>" "Added hunks (all)"]
       ...
       ["fd" "<Cmd>Pick diagnostic scope=\"all\"<CR>" "Diagnostic workspace"]
       ["fD" "<Cmd>Pick diagnostic scope=\"current\"<CR>" "Diagnostic buffer"]
       ["ff" "<Cmd>Pick files<CR>" "Files"]
       ["fg" "<Cmd>Pick grep_live<CR>" "Grep live"]
       ["fG" "<Cmd>Pick grep pattern=\"<cword>\"<CR>" "Grep current word"]
       ["fh" "<Cmd>Pick help<CR>" "Help tags"]
       ["fH" "<Cmd>Pick hl_groups<CR>" "Highlight groups"]
       ["fl" "<Cmd>Pick buf_lines scope=\"all\"<CR>" "Lines (all)"]
       ["fL" "<Cmd>Pick buf_lines scope=\"current\"<CR>" "Lines (buf)"]
       ["fm" "<Cmd>Pick git_hunks<CR>" "Modified hunks (all)"]
       ["fM" "<Cmd>Pick git_hunks path=\"%\"<CR>" "Modified hunks (buf)"]
       ["fr" "<Cmd>Pick resume<CR>" "Resume"]
       ["fR" "<Cmd>Pick lsp scope=\"references\"<CR>" "References (LSP)"]
       ["fs" pick-workspace-symbols-live "Symbols workspace (live)"]
       ["fS" "<Cmd>Pick lsp scope=\"document_symbol\"<CR>" "Symbols document"]
       ["fv" "<Cmd>Pick visit_paths cwd=\"\"<CR>" "Visit paths (all)"]
       ["fV" "<Cmd>Pick visit_paths<CR>" "Visit paths (cwd)"]
       ["la" "<Cmd>lua vim.lsp.buf.code_action()<CR>" "Actions"]
       ["ld" "<Cmd>lua vim.diagnostic.open_float()<CR>" "Diagnostic popup"]
       ...
       ["ls" "<Cmd>lua vim.lsp.buf.definition()<CR>" "Source definition"]
    ...
    (leader-maps xmap-leader
      [["lf" "<Cmd>lua require(\"conform\").format()<CR>" "Format selection"]])))
