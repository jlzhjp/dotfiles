...
(fn toggle-quickfix []
  (let [winid (. (vim.fn.getqflist {:winid true}) :winid)]
    (vim.cmd (if (~= winid 0) "cclose" "copen"))))

(fn toggle-locations []
  (let [winid (. (vim.fn.getloclist 0 {:winid true}) :winid)]
    (vim.cmd (if (~= winid 0) "lclose" "lopen"))))
...
(fn setup-terminal-send-maps []
  (let [get-first-term-chan-id
        (fn []
          (var chan nil)
          (each [_ buf (ipairs (vim.api.nvim_list_bufs))]
            (when (= (. vim.bo [buf] :buftype) "terminal")
              (let [terminal-id (. vim.b [buf] :terminal_job_id)]
        ...
        (fn [lines]
          (let [chan-id (get-first-term-chan-id)]
            (if (not chan-id)
                (vim.notify
                  "No terminal buffer found. Please open a terminal first (e.g., :term)"
                  vim.log.levels.WARN)
                (do
                  (table.insert lines "")
        ...
        (fn []
          (let [view (vim.fn.winsaveview)
            ...
            (if (= found 0)
                (vim.notify "Top-level S-expression not found!" vim.log.levels.WARN)
                (do
                  (vim.cmd "noau normal! \"ty%")
                  (let [text (vim.fn.getreg "t")]
                    (vim.fn.setreg "t" saved-reg)
    ...
    (vim.keymap.set "x" "<LocalLeader>v" send-selection {:desc "Send Selection to Term"})
    (vim.keymap.set "n" "<LocalLeader>s" send-top-sexp {:desc "Send Top Sexp to Term"})))
